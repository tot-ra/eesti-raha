...
export default function App() {
  ...
  const [extraDepth, setExtraDepth] = useState(3);
  const [minFlowValue, setMinFlowValue] = useState(250);
  ...
  const filteredGraph = useMemo(() => {
    ...
    if (!isExpenseNode(focusedNode)) {
      ...
      const descendants = collectDescendants(focusedNode.id, childrenMap);
      const ancestors = collectAncestors(focusedNode.id, parentMap);
      ...
      for (const id of descendants) {
        ...
        if (node.depth <= focusedNode.depth + extraDepth) {
          visible.add(id);
          focusedIds.add(id);
        }
      }
  ...
  const focused = focusNodeId ? activeData.nodes.find((node) => node.id === focusNodeId) : null;
    ...
    <main className="app">
      ...
      <section className="controls">
        <div className="control-card">
          ...
          <div className="hint">Switches full flow graph data</div>
        </div>
      ...
      <BudgetSankey nodes={filteredGraph.nodes} links={filteredGraph.links} onNodeClick={onNodeClick} focusedNodeId={focusNodeId} />
      ...
      <section className="sources">
        ...
        <ul>
          {activeData.meta.sources.map((source) => (
            <li key={source}>
